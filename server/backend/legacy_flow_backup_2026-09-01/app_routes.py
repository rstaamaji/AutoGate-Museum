...
@router.get("/public/payment/{barcode_token}", include_in_schema=True)
def api_public_ticket_payment(
    barcode_token: str,
    db: Session = Depends(get_db),
):
    """QR tiket membuka transaksi Midtrans hanya saat dipindai pengguna."""
    payment = create_ticket_payment(db=db, barcode_token=barcode_token)
    return RedirectResponse(url=payment["redirect_url"])
